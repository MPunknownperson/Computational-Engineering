...
const settingsKey = `${brand.slug}.settings.v3`;
const inputsKey = `${brand.slug}.inputs.v2`;
const historyKey = `${brand.slug}.history.v2`;

/**
 * Reads a stored value under the current namespace, falling back to the key
 * written by the previous brand so returning visitors keep their settings,
 * inputs and history across the rename.
 */
function readStored<T>(key: string, fallback: T): T {
  try {
    const value = localStorage.getItem(key) ?? localStorage.getItem(key.replace(`${brand.slug}.`, `${brand.legacySlug}.`));
...
function loadSettings(): WorkspaceSettings {
  const stored = readStored<Partial<WorkspaceSettings> | null>(settingsKey, null)
    || { ...readStored<Partial<WorkspaceSettings> | null>(`${brand.slug}.settings.v2`, null), appearance: 'light' };
