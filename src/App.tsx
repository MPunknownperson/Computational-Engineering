...
import { ArrowRight, Bookmark, Check, ChevronRight, CircuitBoard, Globe2, History, Info, Landmark, ListOrdered, Menu, Settings2, X } from 'lucide-react';
...
const primaryLinks: { hash: string; label: string }[] = [
  { hash: 'calculators', label: 'Calculators' },
  { hash: 'converters', label: 'Converters' },
  { hash: 'guides', label: 'Guides' },
...
export default function App() {
  ...
  const activeTop = route.name === 'directory' ? (group === 'Converters' ? 'converters' : 'calculators') : route.name;
  ...
  return <div className="site">
    ...
    <header className="site-header">
      <div className="header-inner">
        <a className="site-brand" {...linkProps('')} aria-label="CalcVault home"><BrandGlyph /><span>calc<span className="brand-blue">vault</span><span className="brand-period">.</span></span></a>
        <nav className="primary-nav" aria-label="Main navigation">
          {primaryLinks.map(link => <a key={link.hash} {...linkProps(link.hash)} className={activeTop === link.hash || (link.hash === 'calculators' && route.name === 'calculator') ? 'active' : ''}>{link.label}</a>)}
        </nav>
      ...
      <footer className="site-footer">
        <div className="footer-grid">
          <div className="footer-brand">
            <BrandGlyph />
            <p>Free calculators with dated regional references. No account, no tracking, no advertising.</p>
          </div>
        ...
        <p className="footer-legal">Planning estimates only. Not tax, legal, lending or investment advice. Reference packs are dated and may not reflect the latest amendments.</p>
      </footer>
    </main>
    ...
    {modal === 'navigation' && <Dialog title="Explore CalcVault" className="navigation-dialog" onClose={() => setModal(null)}>
      <nav className="mobile-navigation" aria-label="Mobile navigation">
        ...
        <button onClick={() => navigate('guides')}><ArrowRight size={20} /><span>Guides</span><ChevronRight size={17} /></button>
