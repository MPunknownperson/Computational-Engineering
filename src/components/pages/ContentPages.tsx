...
export function AboutPage({ onNavigate }: { onNavigate: (hash: string) => void }) {
  return <div className="content-page route-enter">
    <header className="page-intro">
      <h1><Info size={25} />About {brand.name}</h1>
    ...
    <section className="article-section">
      <h2>The name</h2>
      <p>
        {brand.name} is a coined word: <em>arith-</em>, from arithmetic, and <em>-vale</em>, a valley
        &mdash; a place where things settle and can be seen clearly. It was chosen because it is
        original and distinctive, so that searching for it leads here and nowhere else, and because it
        describes the site&rsquo;s one promise: every number is worked out in the open.
      </p>
    </section>
    ...
    <section className="article-section">
      ...
      <p>
        Use of the site is governed by the Terms and Conditions, and the way information is handled
        &mdash; almost entirely on your own device &mdash; is set out in the Privacy Policy. Corrections
        to reference data, accessibility feedback and privacy requests all go through the Contact page.
      </p>
      <div className="hero-actions">
        <button className="secondary-button" onClick={() => onNavigate('terms')}>Terms and Conditions</button>
        <button className="secondary-button" onClick={() => onNavigate('privacy')}>Privacy Policy</button>
        <button className="secondary-button" onClick={() => onNavigate('contact')}>Contact</button>
      </div>
    </section>
  </div>;
}
