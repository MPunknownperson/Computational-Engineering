...
export function NumberField({ label, value, onChange, prefix, suffix, hint, labelAction, className = '', locale = 'en-US', field, error, showHint = false }: { label: string; value: string; onChange: (value: string) => void; prefix?: string; suffix?: ReactNode; hint?: string; labelAction?: ReactNode; className?: string; locale?: string; field?: string; error?: string; showHint?: boolean }) {
  const id = useId();
  ...
  return <div className={`field ${className}`} data-field={field}>
    ...
    <div className={`input-wrap ${focused ? 'is-focused' : ''} ${invalid ? 'is-invalid' : ''}`}>
      ...
      <input id={id} type="text" inputMode="decimal" value={display} onFocus={() => setFocused(true)} onBlur={() => { setFocused(false); if (Number.isFinite(numeric) && value !== String(numeric)) onChange(String(numeric)); }} onChange={event => onChange(normalizeNumberInput(event.target.value,locale))} autoComplete="off" spellCheck={false} aria-invalid={invalid} aria-describedby={error ? `${id}-error` : hint ? `${id}-hint` : undefined} />
...
export function Toggle({ label, checked, onChange, description }: { label: string; checked: boolean; onChange: (value: boolean) => void; description?: string }) {
  return <label className="toggle-row"><input type="checkbox" checked={checked} onChange={event => onChange(event.target.checked)} /><span><span className="toggle-label">{label}</span>{description && <span className="toggle-description">{description}</span>}</span></label>;
}