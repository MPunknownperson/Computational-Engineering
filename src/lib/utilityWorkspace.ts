...
const KEY = 'calcvault.workspace.v1';
const LIMIT = 120;
...
export function exportWorkspace(workspace: UtilityWorkspace) {
  return JSON.stringify({ format: 'calcvault-workspace', version: 1, exportedAt: new Date().toISOString(), workspace }, null, 2);
}
...
export function importWorkspace(json: string): { workspace: UtilityWorkspace | null; message: string } {
  try {
    ...
    if (parsed.format !== 'calcvault-workspace' || !parsed.workspace) return { workspace: null, message: 'That file is not a CalcVault workspace export.' };
