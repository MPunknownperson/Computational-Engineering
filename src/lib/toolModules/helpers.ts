...
export const node = (id: string, label: string, role: string, value: number | string, format: NetworkNode['format'], formulaNote: string, connections: string[] = []): NetworkNode => ({ id, label, role, institution: 'CalcVault deterministic engine', value, format, formulaNote, connections });
